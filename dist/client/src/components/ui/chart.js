...
const ChartTooltipContent = React.forwardRef((props, ref) => {
    const { active, className, indicator = "dot", hideLabel = false, hideIndicator = false, label, labelFormatter, labelClassName, formatter, color, nameKey, labelKey, payload = [], ...divProps } = props;
    const { config } = useChart();
    // recharts TooltipProps has payload: any[]
    const typedPayload = (divProps.payload ?? []);
    const tooltipLabel = React.useMemo(() => {
        if (hideLabel || !typedPayload || !typedPayload.length) {
            return null;
        }
        ...
        const value = !labelKey && typeof label === "string"
            ...
            : itemConfig?.label;
        if (labelFormatter) {
            return (_jsx("div", { className: cn("font-medium", labelClassName), children: labelFormatter(value, typedPayload) }));
        }
        if (!value) {
            return null;
        }
    ...
    return (_jsxs("div", { ref: ref, className: cn("grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl", className), ...divProps, children: [!nestLabel ? tooltipLabel : null, _jsx("div", { className: "grid gap-1.5", children: typedPayload.map((item, index) => {
                    ...
                    return (_jsx("div", { className: cn("flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5 [&>svg]:text-muted-foreground", indicator === "dot" && "items-center"), children: formatter && item?.value !== undefined && item.name ? (formatter(item.value, item.name, item, index, item.payload)) : (_jsxs(_Fragment, { children: [itemConfig?.icon ? (_jsx(itemConfig.icon, {})) : (!hideIndicator && (_jsx("div", { className: cn("shrink-0 rounded-[2px] border-[--color-border] bg-[--color-bg]", {
                                    ...
                                    } }))), _jsxs("div", { className: cn("flex flex-1 justify-between leading-none", nestLabel ? "items-end" : "items-center"), children: [_jsxs("div", { className: "grid gap-1.5", children: [nestLabel ? tooltipLabel : null, _jsx("span", { className: "text-muted-foreground", children: itemConfig?.label || item.name })] }), item.value != null && (_jsx("span", { className: "font-mono font-medium tabular-nums text-foreground", children: Number(item.value).toLocaleString() }))] })] })) }, item.dataKey ?? index));
                }) })] }));
});
ChartTooltipContent.displayName = "ChartTooltip";
const ChartLegend = RechartsPrimitive.Legend;
const ChartLegendContent = React.forwardRef(({ className, hideIcon = false, payload, verticalAlign = "bottom", nameKey, ...divProps }, ref) => {
    const { config } = useChart();
    const typedPayload = (divProps.payload ?? []);
    if (!typedPayload || !typedPayload.length) {
        return null;
    }
    return (_jsx("div", { ref: ref, className: cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className), ...divProps, children: typedPayload.map((item, index) => {
