...
async function fetchHackerNews() {
    try {
        const resp = await axios.get("https://hn.algolia.com/api/v1/search?query=stock+market+finance+bitcoin+fed+interest+rates&tags=story&hitsPerPage=20&numericFilters=points>10", { timeout: 8000 });
...
async function fetchRedditQuant() {
    try {
        const resp = await axios.get("https://www.reddit.com/r/quant/new.json?limit=25", {
            timeout: 8000,
            headers: { "User-Agent": "MarketPulse/1.0 (educational)" },
        });
        return resp.data.data.children.map((p) => ({
            title: p.data.title,
...
async function fetchRedditInvesting() {
    try {
        const resp = await axios.get("https://www.reddit.com/r/investing/hot.json?limit=15", {
            timeout: 8000,
            headers: { "User-Agent": "MarketPulse/1.0 (educational)" },
        });
        return resp.data.data.children.map((p) => ({
...
async function fetchBlueSky() {
    try {
        const resp = await axios.get("https://public.api.bsky.app/xrpc/app.bsky.feed.searchPosts?q=stock+market+fed+inflation&limit=20", { timeout: 8000 });
        return (resp.data.posts || []).map((p) => ({
...
async function fetchYahooFinanceRSS() {
    try {
        const resp = await axios.get("https://feeds.finance.yahoo.com/rss/2.0/headline?s=SPY,QQQ,BTC-USD&region=US&lang=en-US", { timeout: 8000 });
...
export async function runScrub(geminiApiKey) {
