...
import { generateEli5, generatePredictiveReasoning, } from "./geminiService.js";
...
export async function registerRoutes(app) {
    ...
    app.get("/api/chart/:ticker", async (req, res) => {
        try {
            ...
            const candles = await getChart(req.params.ticker.toUpperCase(), range, interval);
            res.json(candles);
        }
        catch (error) {
    ...
    app.get("/api/vectors/:ticker", async (req, res) => {
        const vectors = await storage.getVectorsByTicker(req.params.ticker.toUpperCase());
        res.json(vectors);
    });
