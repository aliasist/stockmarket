...
async function fetchHackerNews() {
    const data = await fetchJson("https://hn.algolia.com/api/v1/search?query=stock+market+finance+bitcoin+fed+interest+rates&tags=story&hitsPerPage=20&numericFilters=points>10");
...
async function fetchBlueSky() {
    const data = await fetchJson("https://public.api.bsky.app/xrpc/app.bsky.feed.searchPosts?q=stock+market+fed+inflation&limit=20");
...
async function fetchYahooFinanceRSS() {
    const text = await fetchText("https://feeds.finance.yahoo.com/rss/2.0/headline?s=SPY,QQQ,BTC-USD&region=US&lang=en-US");
    if (!text) {
        return [];
    }
...
export async function runScrub(env) {
