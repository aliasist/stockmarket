...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center h-full text-center px-8 py-16 gap-6">
      {/* UFO SVG */}
      <svg viewBox="0 0 100 58.4275" width={90} style={{ filter: "drop-shadow(0 0 16px rgba(10,203,155,0.5))", animation: "floatUfo 3s ease-in-out infinite" }}>
        <defs>
          <linearGradient id="pitch-ufo-grad" x1="0%" y1="0%" x2="100%" y2="100%">
        ...
        <path
          ...
          fill="url(#pitch-ufo-grad)"
        />
      </svg>
      <style>{`@keyframes floatUfo { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-8px)} }`}</style>

      <div>
        <div className="text-foreground font-semibold text-base mb-1">Generate Your First Pitch</div>
        <div className="text-muted-foreground text-sm leading-relaxed max-w-xs">
          Enter a stock ticker to generate a professional investment memo powered by real market data and Groq AI.
        </div>
      </div>

      <div className="flex items-center gap-4 text-[11px] font-mono text-muted-foreground/60">
        <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-primary inline-block" />Real FMP data</span>
        <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-primary/60 inline-block" />Groq AI narrative</span>
        <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-primary/40 inline-block" />Print-ready</span>
      </div>
    </div>
  );
}
