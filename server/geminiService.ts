...
export async function generateMarketVectors(
  articles: AnalyzedArticle[],
  apiKey: string
): Promise<MarketVectorResult[]> {
  const model = getModel(apiKey);
...
export async function generateEli5(term: string, apiKey: string): Promise<string> {
  const model = getModel(apiKey);
...
export async function generatePredictiveReasoning(
  ticker: string,
  recentVectors: Array<{ signal: string; confidence: number; reasoning: string; createdAt: string }>,
  recentNews: Array<{ title: string; tone?: string }>,
  apiKey: string
): Promise<{ prediction: string; confidence: number; reasoning: string; timeframe: string }> {
  const model = getModel(apiKey);
