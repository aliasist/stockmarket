...
if (watchlistCount.c === 0) {
  ...
  const seedMany = db.transaction((items) => {
    for (const item of items) insert.run(item.ticker, item.name, item.type);
  });
  seedMany([
...
const db_watchlist = {
  ...
  add: (ticker, name, type) =>
    db.prepare('INSERT INTO watchlist (ticker, name, type) VALUES (?, ?, ?)').run(ticker, name, type),
...
const db_scrub = {
  createRun: () =>
    db.prepare("INSERT INTO scrub_runs (status) VALUES ('running')").run(),

  updateRun: (id, { vectorsFound, status, summary, sources }) =>
    db
      .prepare(
        `UPDATE scrub_runs
  ...
  getLatest: () =>
    db.prepare('SELECT * FROM scrub_runs ORDER BY id DESC LIMIT 1').get(),

  getAll: () =>
    db.prepare('SELECT * FROM scrub_runs ORDER BY id DESC LIMIT 50').all(),
};
...
const db_articles = {
  ...
  getLatestBySource: (source, limit = 20) =>
    db
      .prepare(
        `SELECT a.* FROM articles a
         INNER JOIN scrub_runs r ON a.scrubRunId = r.id
...
const db_vectors = {
  insert: ({ scrubRunId, ticker, signal, confidence, reasoning, sources }) =>
    db
      .prepare(
        `INSERT INTO vectors (scrubRunId, ticker, signal, confidence, reasoning, sources)
         VALUES (?, ?, ?, ?, ?, ?)`
      )
      .run(
        scrubRunId,
  ...
  getAll: () =>
    db.prepare('SELECT * FROM vectors ORDER BY id DESC LIMIT 100').all(),

  getLatest: () =>
    db.prepare('SELECT * FROM vectors ORDER BY id DESC LIMIT 10').all(),
};
...
const db_eli5 = {
  ...
  set: (term, explanation) => {
    ...
    return db
      .prepare(
        `INSERT INTO eli5_cache (term, explanation)
         VALUES (?, ?)
