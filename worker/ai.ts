...
Return JSON only.`
    ...
    try {
      const parsed = JSON.parse(cleanJsonPayload(text)) as Array<
        Pick<AnalyzedArticle, "tone" | "toneReasoning" | "ticker" | "summary">
      >
...
export async function generateVectorsWithAI(
  env: Env,
    ...
    return [
      {
        ...
        confidence: 0.5,
        reasoning:
          "No AI key is configured, so this vector is a neutral fallback rather than a synthesized market read.",
...
${input.recentVectors
  ...
  .map(
    (vector) =>
      `- ${vector.signal.toUpperCase()} (${Math.round(vector.confidence * 100)}%): ${vector.reasoning}`
  )
  .join("\n")}
